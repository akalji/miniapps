...
export class LanguageService {
  ...
  setLanguage(value: string): void {
    ...
    try {
      this.document.defaultView?.localStorage.setItem('jar-labler.language', value);
    } catch {
  ...
  private detect(): Language {
    ...
    try {
      const stored =
        window?.localStorage.getItem('jar-labler.language') ??
        window?.localStorage.getItem('pickle-label.language');
